...
class Main(Scene):
    ...
    def add_main(self):
        ...
        self.info = Info("")
        self.info_update("精英选择: 对种群按适应度大小排序，返回前 #size 个", pos=LEFT*3+DOWN*3)
        self.wait(1.5)
...
class Test(Scene):
    def construct(self):
        ...
        while alive_snakes_set:
            ...
            self.matrix[game.food[1]][game.food[0]].set_fill(PURE_RED, opacity=1)
            for snake in alive_snakes:
                for body in snake.body[1:]:
                    self.matrix[body[1]][body[0]].set_fill(snake.color, opacity=1)
                self.matrix[snake.body[0][1]][snake.body[0][0]].set_fill(WHITE, opacity=1)
            self.wait(0.05)
