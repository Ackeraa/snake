...
class Main(Scene):
    ...
    def add_flow(self):
        ...
        for x in dies:
            self.play(FadeOut(population[x]), run_time=0.5)
    ...
    def add_code_pic(self):
        ...
        t1 = Text("实现贪吃蛇", font_size=16, color=RED).next_to(title1, UP)
        t2 = Text("实现神经网络", font_size=16, color=RED).next_to(title2, UP)
        t3 = Text("实现遗传算法", font_size=16, color=RED).next_to(title3, UP)
    ...
    def add_ai_game(self):
        ...
        title = MathTex("ai\_game.py", font_size=24).set_color([BLUE_E, TEAL_D]).shift(UP*3.5)
        anims = []
        ...
        self.info_update(infos[0], pos=RIGHT*4, font_size=16)
        for i in range(1, len(lines)):
            self.play(arr.move(lines[i]))
            self.info_update(infos[i], pos=RIGHT*4, font_size=16)
            self.wait(1.5)
        # snake
        ## __init__
        ...
        self.info_update("初始化蛇的位置、方向、得分等", pos=RIGHT*4, font_size=16)
        self.wait(1.5)
        self.play(Indicate(code.code[16], color=RED))
        self.info_update("用于判断蛇是否进入死循环", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.play(Indicate(code.code[17], color=RED))
        self.info_update("根据基因初始化神经网络", pos=RIGHT*4, font_size=16)
        self.wait(2)
        ...
        self.info_update("蛇头方向", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.info_update("")
        self.play(Create(code.code[15:35]), run_time=8)
        self.info_update("蛇头8个方向上是否有食物、自身、墙", pos=RIGHT*4, font_size=16)
        self.wait(2)
        ...
        self.info_update("初始化界面大小", pos=RIGHT*4, font_size=16)
        self.wait(1)
        self.play(Indicate(code.code[11:13], color=RED))
        self.info_update("初始化随机函数(用于训练过程的复现)", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.play(Indicate(code.code[14:21], color=RED))
        self.info_update("根据基因列表生成若干条蛇", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.play(Indicate(code.code[22], color=RED))
        self.info_update("生成初始食物", pos=RIGHT*4, font_size=16)
        self.wait(1)
        ...
        arr.move(11, False)
        self.play(FadeIn(arr))
        self.info_update("当还有蛇存活时", pos=RIGHT*4, font_size=16)
        self.wait()
        self.play(arr.move(13))
        self.info_update("每条蛇依次移动", pos=RIGHT*4, font_size=16)
        self.wait()
        self.play(arr.move(15))
        self.info_update("如果食物被吃掉，生成新食物", pos=RIGHT*4, font_size=16)
        self.wait(1.5)
        self.play(arr.move(16))
        self.info_update("舍弃死亡的蛇", pos=RIGHT*4, font_size=16)
        self.wait(1.5)
        self.play(arr.move(23))
        self.info_update("返回得分、步数及随机数种子", pos=RIGHT*4, font_size=16)
        self.wait(1.5)
    ...
    def add_nn(self):
        ...
        self.play(arr.move(5))
        self.play(arr.move(6))
        t_in = Text("1", font_size=18, color=GREEN_C).move_to(nn[0][0])
        self.play(arr.move(7))
    ...
    def add_main(self):
        ...
        # Individual
        self.play(FadeOut(code.code[13:37], shift=DOWN), FadeOut(arr))
        ...
        self.info_update("初始化个体基因、得分、步数、随机数种子", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.play(Indicate(code3[14:17], color=RED))
        self.info_update("进行一轮游戏，根据得分、步数计算适应度", pos=RIGHT*4, font_size=16)
        self.wait(2)
        code4 = MyCode("main_4.py").code.shift(LEFT * 3)
        code.code[5:13] = code4[5:13]
        self.play(FadeOut(code3[7:18], shift=DOWN),
        ...
        self.info_update("初始化种群大小、基因长度、变异概率等", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.play(Indicate(code.code[17:20], color=RED))
        self.info_update("生成初始种群, 基因是(-1,1)区间均匀分布", pos=RIGHT*4, font_size=16)
        self.wait(2)
        self.play(FadeOut(code.code[5:21], shift=DOWN))

        ## crossover
        code5 = MyCode("main_5.py").code.shift(LEFT * 3)
        code.code[7:17] = code5[7:17]   
        ...
        self.info_update("精英选择: 对种群按适应度大小排序，返回前 #size 个", pos=RIGHT*4, font_size=16)
        self.wait(1.5)
        arr.move(12, False)
        self.play(FadeIn(arr))
        self.info_update("轮盘赌选择: 适应度越大被选概率越大", pos=RIGHT*4, font_size=16)
        self.wait(1.5)
        ...
        arr.move(7, False)
        self.play(FadeIn(arr))
        self.info_update("进化", pos=RIGHT*4, font_size=16)
        self.play(FadeOut(arr))
        self.wait(1)
        self.play(Indicate(code.code[8:13], color=RED))
        self.info_update("计算个体适应度，并选取适应度高的个体作为父代", pos=RIGHT*4, font_size=16)
        self.wait(3)
        self.play(Indicate(code.code[15:23], color=RED))
        self.info_update("每次轮盘赌选择两个父代个体，并交叉、变异生成两个子代个体", pos=RIGHT*4, font_size=16)
        self.wait(3)
    ...
    def construct(self):
        ...
        for i in range(n):
            ...
            with open(genes_pth, "r") as f:
                genes = np.array(list(map(floatt, f.read().splitt())))
